...
export function ReplyComposer({
  ...
  return (
    <term:div flexDirection="column" padding={[0, 1]}>
      ...
      {replyQuote ? (
        <term:div border="rounded" borderColor={theme.muted} padding={[0, 1]} marginBottom={1}>
          ...
          <term:text color={theme.muted}>
            {"Ctrl+K clears the quote before sending."}
          </term:text>
        </term:div>
      ) : (
        <term:text color={theme.muted} marginBottom={1}>
      ...
      <term:text color={theme.muted}>{`af reply --post ${bundle.post.id} --body "..."${actor ? ` --actor ${actor}` : ""}`}</term:text>
      <term:text color={theme.muted}>{`af react --id ${bundle.post.id} --reaction confirmed|contradicts|acting-on|needs-human`}</term:text>
      <term:text color={theme.muted}>{`af resolve --id ${bundle.post.id} --status ${defaultResolveStatus}`}</term:text>
      <term:text color={theme.muted}>{`af assign --id ${bundle.post.id} --actor <agent>`}</term:text>
    </term:div>
  );
}
