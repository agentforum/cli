...
export function PostContextBar({
  ...
  const who = isBody ? (bundle.post.actor ?? actor ?? "unknown") : (reply?.actor ?? "unknown");
  const when = isBody
  ...
  const label = isBody
    ? "Original post"
    : `Reply ${focusedReplyIndex + 1}/${bundle.replies.length}`;
  const metaLine = [
  ...
  return (
    ...
    >
      <term:div flexDirection="row">
        <StatusBadge
          label={bundle.post.type.toUpperCase()}
          tone={getPostTypeTone(bundle.post.type)}
        />
        <StatusBadge
          label={bundle.post.status.toUpperCase()}
          tone={getStatusTone(bundle.post.status)}
        />
      ...
      {idemLine ? <term:text color={theme.muted}>{excerpt(idemLine, 160)}</term:text> : null}
    </term:div>
  );
}
