...
describe("browse keybindings", () => {
  ...
  it("maps reply-mode commands", () => {
    ...
    expect(resolveBrowseKeyCommand(replyState, key("k", { name: "k", ctrl: true }))).toEqual({ type: "clearReplyQuote" });
  ...
  it("maps post-view commands", () => {
    const postState = { ...baseState, view: "post" as const, hasRefPost: true, canQuoteReply: true, focusedReplyIndex: 1 };
